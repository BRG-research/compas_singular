...
def modify_density(compas_singularity_mesh):

	skeys = select_mesh_strips(compas_singularity_mesh, show_density=True)
	...
	if densification_type == 'density_value':
		...
		compas_singularity_mesh.set_strips_density(d, skeys)

	elif densification_type == 'target_edge_length':
		t = rs.GetReal('target edge length', number=1.)
		compas_singularity_mesh.set_strip_density_target(t, skeys)

	elif densification_type == 'target_face_number':
		nb_faces = rs.GetInteger('target face number', number=100, minimum=1)
		compas_singularity_mesh.set_strips_density_equal_face_target(nb_faces)

	return compas_singularity_mesh


def update_density(compas_singularity_mesh):

	compas_singularity_mesh.densification()
