...
from compas_pattern.datastructures.mesh_quad.grammar import add_strip
from compas_pattern.datastructures.mesh_quad.grammar import delete_strips
from compas_pattern.datastructures.mesh_quad.grammar import split_strip
from compas_pattern.datastructures.mesh_quad.grammar import strips_to_split_to_preserve_boundaries_before_deleting_strips
...
def singular():
	...
	if pattern_from == 'coarse_quad_mesh':
		...
		coarse_quad_mesh = surface_decomposition(guid, rs.GetReal('precision', number = 1.), output_skeleton = False)[0]
		coarse_quad_mesh.init_strip_density()
		coarse_quad_mesh.quad_mesh = coarse_quad_mesh.copy()
		coarse_quad_mesh.polygonal_mesh = coarse_quad_mesh.copy()
	...
	while True:
		...
		if operation == 'add':
			...
			for skey, n in to_split.items():
				...
				d = int(ceil(float(coarse_quad_mesh.get_strip_density(skey)) / 2.))
